...
export const apiSearchController = async (req: Request, res: Response) => {
  ...
  try {
    const response: AxiosResponse<DaDataResponse> = await axios.post(API_URL, requestData, {
      ...
      timeout: 10000, // Установка таймаута гарантирует, что ваше приложение не застрянет в ожидании внешнего сервиса, который может быть нерабочим или медленным. В данном случае таймаут: 5000 устанавливает 5-секундный лимит для завершения запроса.
    });
